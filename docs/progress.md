...
- [x] 관계·구조 탭 박스+화살표 다이어그램(유형별 색상) ✅
- [x] 본문 위 SVG 곡선 화살표(인과·과정 한방향, 비교대조 양방향) ✅
- [x] **손그림 입력(하이브리드)** ✅ 검증: 밑줄·동그라미(획→글자 인식), 관계연결(가까운 표시 자동), 나열(순서대로 탭→1·2·3), 지우기. 도구 없으면 읽기·스크롤. — 실제 태블릿 손가락/펜 검증은 추후 권장
...
### M3 에이전트 진단·피드백 (예정)
### 배포 / 운영
- [x] Vercel 배포 완료: https://reading-1kvv.vercel.app (env 5개 설정, GitHub 연동 자동배포)
- [x] 학생 진행·기록: 세션 완료/다시읽기 표시 + /read '내 읽기 기록' 목록

### M4 교사 대시보드 (진행 중)
- [x] 학생 활동 목록(/teacher/activity): 학생·지문·표시/관계/설명 수·상태
- [x] 활동 상세(/teacher/activity/[id]): 표시된 지문(읽기전용) + 관계 목록 + 자기설명·코치 대화
- [x] 0006: 교사 읽기 RLS(프로필/세션/표시/설명/코치대화) — 파일럿(전체), 추후 학급 범위
- [x] 0006 적용 + 대시보드 실제 검증 완료 ✅
- [x] 학급·참여코드: 교사 학급 생성(코드 발급)/삭제, 학생 코드 참여(join_class RPC), 대시보드 학급 열
- [x] 0007 적용 + 학급 생성·참여 검증 완료 ✅
- [ ] (나중) 활동을 학급별로 필터, AI 진단 결과 표시
### M5 검증·튜닝 (예정)
### M6 배포·파일럿 (예정)
